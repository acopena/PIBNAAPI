...
namespace PIBNAAPI.Command.action
{
    ...
    {
        public async Task<TeamDashboardModel> GetTeamSummary(int? seasonId)
        {
            ...
            using (var ctx = new PIBNAContext())
            {
                if (seasonId == null)
                {
                    var CurrentSeason = await ctx.PSeason
                ...
                header.DivisionInfo = await (from p in ctx.PDivision
                                             where p.EndDate == null
                ...
                var teams = ctx.PTeam.Where(s => s.SeasonId == seasonId.Value && s.EndDate == null).ToList();
        ...
        public async Task<List<ClubModel>> GetList()
        {
            ...
            try
            {
                using (var ctx = new PIBNAContext())
                {
                    ...
                    foreach (var i in data)
                    {
                        ...
                        var oList = await ctx.PClubOfficial.Where(s => s.ClubId == i.ClubId && s.EndDate == null).ToListAsync();
        ...
        public async Task<ClubPageModel> GetListByPage(int pageSize, int page)
        {
            var model = new ClubPageModel();
            using (var ctx = new PIBNAContext())
            {
                var clubs = from p in ctx.PClub
                            where p.EndDate == null
        ...
        public async Task<ClubModel> GetById(int id)
        {
            ...
            using (var ctx = new PIBNAContext())
            {
                ...
                if (data != null)
                {
                    ...
                    var officials = await ctx.PClubOfficial.Where(s => s.ClubId == id && s.EndDate == null).Select(s => s).ToListAsync();
